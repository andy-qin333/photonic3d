...
public class IOUtilities {
	...
	public static byte[] read(SerialCommunicationsPort currentIdentifier, int timeoutMillis, int cpuLimitingDelay) throws IOException, InterruptedException {
		byte[] bytes = null;
		long start = System.currentTimeMillis();
		while (System.currentTimeMillis() - start < timeoutMillis) {
			byte[] data = currentIdentifier.read();
			if (data == null) {
				...
				bytes = BasicUtillities.bytesCat(bytes, data);
				start = System.currentTimeMillis();
			}
			Thread.sleep(cpuLimitingDelay);
		}
	...
	public static byte[] readBytes(SerialCommunicationsPort currentIdentifier, int size, int timeoutMillis, int cpuLimitingDelay) throws IOException, InterruptedException {
		byte[] bytes = null;
		int readsize = size;
		long start = System.currentTimeMillis();
		while (System.currentTimeMillis() - start < timeoutMillis) {
			byte[] data = currentIdentifier.readBytes(readsize);
			if (data == null) {
				if (bytes != null) {
					break;
				}
