...
public class HostProperties {
	...
	// FIXME: 2017/9/1 zyd add for udisk file -s
	private File udiskDir;
	// FIXME: 2017/9/1 zyd add for udisk file -e
	...
	// FIXME: 2017/9/6 zyd add for enable system update -s
	...
	private HostProperties() {
		...
		// FIXME: 2017/9/1 zyd add for udisk file -s
		udiskDirString = configurationProperties.getProperty("udiskdir");
		// FIXME: 2017/9/1 zyd add for udisk file -e
		...
		// FIXME: 2017/9/6 zyd add for enable system update -s
		enableUpdate = new Boolean(configurationProperties.getProperty("enableUpdate", "false"));
		// FIXME: 2017/9/6 zyd add for enable system update -e
		// FIXME: 2018/6/27 zyd add for get model number -s
		modelNumber = configurationProperties.getProperty("modelNumber", "3DTALK_DS100");
		// FIXME: 2018/6/27 zyd add for get model number -e
		// FIXME: 2018/9/3 zyd add for GraphicsMagick -s
		...
		// FIXME: 2017/9/1 zyd add for udisk file -s
		if (udiskDirString == null) {
			udiskDir = new File(System.getProperty("user.home"), "udiskdir");
		} else {
			udiskDir = new File(udiskDirString);
		}
		// FIXME: 2017/9/1 zyd add for udisk file -e
		...
		if (!udiskDir.exists()) {
			try {
				FileUtils.forceMkdir(udiskDir);
			} catch (IOException e) {
				throw new IllegalArgumentException("Couldn't create udisk directory", e);
			}
		}
		// FIXME: 2017/9/1 zyd add for udisk file -e
	...
	// FIXME: 2018/6/27 zyd add for get model -s
	public String getModelNumber() {
		return modelNumber;
	}
	// FIXME: 2018/6/27 zyd add for get model -e
	...
	public ParameterRecord getParameterRecord()
	{
		...
		{
			logger.error("Problem marshalling parameter record from:" + recordFile, e);
		}
		return parameterRecord;
	}

	public void saveParameterRecord(ParameterRecord record)
	{
		JAXBContext jaxbContext;
		try
		{
			...
			logger.error("Problem saving parameter record for:" + parameterRecord, e);
		}
	}
	// FIXME: 2017/10/10 zyd add for record parameter -e
