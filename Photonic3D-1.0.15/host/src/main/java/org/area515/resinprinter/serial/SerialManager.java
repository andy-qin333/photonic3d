...
public class SerialManager {
	...
	private DetectedResources detectResourcesAndAssignPort(Printer printer, SerialCommunicationsPort identifier, ComPortSettings printerOverriddenComPortSettings, ComPortReservation reservationStyle) throws AlreadyAssignedException, InappropriateDeviceException {
		...
		// FIXME: 2017/9/1 zyd add for uartscreen -s
		if (reservationStyle == ComPortReservation.UartScreen) {
			resources = getProjectorModel(identifier, currentlyOverridenSettings);
		}
		// FIXME: 2017/9/1 zyd add for uartscreen -e
	...
	// FIXME: 2017/9/1 zyd add for uartscreen -s
	public void assignSerialPortToUartScreen(Printer printer, SerialCommunicationsPort identifier) throws AlreadyAssignedException, InappropriateDeviceException {
		ComPortSettings settings = printer.getConfiguration().getMachineConfig().getUartScreenConfig().getComPortSettings();

		DetectedResources resources = detectResourcesAndAssignPort(printer, identifier, settings, ComPortReservation.UartScreen);
		identifier = resources.comPort;
		...
		try {
			identifier.open(printer.getName(), OPEN_TIME_OUT, resources.settings);
			printer.setUartScreenSerialPort(identifier);
	...
	public void removeAssignments(Printer printer) {
		...
		// FIXME: 2017/9/1 zyd add for uartscreen -s
		SerialCommunicationsPort uartScreenPort = printer.getUartScreenSerialPort();
		// FIXME: 2017/9/1 zyd add for uartscreen -e
		...
		if (uartScreenPort != null) {
			logger.info("Removing uartScreen serial:{} assignment for printer:{}", uartScreenPort, printer);
			printersBySerialPort.remove(uartScreenPort);
			printer.setUartScreenSerialPort(null);
		}
		// FIXME: 2017/9/1 zyd add for uartscreen -e
	...
	// FIXME: 2018/7/20 zyd add for firmware install -s
	public void removeAssignmentFirmware(Printer printer)
	{
		if (printer == null)
			return;
		SerialCommunicationsPort firmwarePort = printer.getPrinterFirmwareSerialPort();
	...
	// FIXME: 2018/7/20 zyd add for firmware install -e
