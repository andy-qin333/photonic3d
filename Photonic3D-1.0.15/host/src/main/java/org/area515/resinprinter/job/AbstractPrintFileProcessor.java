...
public abstract class AbstractPrintFileProcessor<G,E> implements PrintFileProcessor<G,E>{
	...
	public void parseConfigFile(DataAid aid, File cfgFile)
	{
		...
		try
		{
			...
			double volume = new Double(properties.getProperty("volume","0")); 
			aid.printer.setVolume(volume);
			//2019-11-11 derby add for sum of material;set the resine volume for this job 
	...
	public void parseConfigFileChange(DataAid aid, com.alibaba.fastjson.JSONArray jsonArray,int i ,File cfgFile)
	{
		
//		JSONObject jsonObject = jsonArray.getJSONObject(i);

//		double sliceHeight = new Double(jsonObject.getDouble("layer_height"));
//		if (sliceHeight > 0)
//			aid.inkConfiguration.setSliceHeight(sliceHeight);
//		
//		if (!aid.slicingProfile.getParameterEnabled())
...
//			
//			int layerTime = new Integer(jsonObject.getInt("exposure_time"));
//			double liftDistance = new Double(jsonObject.getDouble("lift_distance"));
//			
//			aid.inkConfiguration.setExposureTime(layerTime);
//			aid.slicingProfile.setLiftDistance(liftDistance);
//		}
...
//
		...
		try
		{
			...
			double volume = new Double(properties.getProperty("volume","0")); 
			double preSumOfMaterial = aid.printer.getSumOfMaterial();
			aid.printer.setSumOfMaterial(volume+preSumOfMaterial);
			//2019-11-11 derby add for sum of material
		...
		try {
			...
			double sliceHeight = (double)subObject.get("layer_height");
			if (sliceHeight > 0)
				aid.inkConfiguration.setSliceHeight(sliceHeight);
			
			if (!aid.slicingProfile.getParameterEnabled())
			{	
				int layerTime = (int)subObject.get("exposure_time");
				double liftDistance = (double)subObject.get("lift_distance");
	...
	/////////add by derby9-28 for ds300 moving uvled
	/////////printImageAndPerformPostProcessing(DataAid aid, RenderedData imageData)
	public JobStatus printImageAndPerformPostProcessing(DataAid aid, RenderedData imageData) throws ExecutionException, InterruptedException, InappropriateDeviceException, ScriptException {
		if (aid == null) {
			throw new IllegalStateException("initializeDataAid must be called before this method");
		}
		...
		aid.printer.showImage(imageData.getSplitImage1()); //derby add for insure the img is ready
	...
	public JobStatus printImageAndPerformPostProcessing(DataAid aid, BufferedImage image) throws ExecutionException, InterruptedException, InappropriateDeviceException, ScriptException {
		if (aid == null) {
			throw new IllegalStateException("initializeDataAid must be called before this method");
		}
		...
		aid.printer.showImage(image); //derby add for insure the img is ready
