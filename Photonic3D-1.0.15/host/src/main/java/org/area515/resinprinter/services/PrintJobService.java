...
public class PrintJobService {
	...
	@ApiOperation(value="Overrides the lift feed speed variable for the PrintJob designated by the specified job id.")
    @ApiResponses(value = {
            @ApiResponse(code = 200, message = SwaggerMetadata.SUCCESS),
            @ApiResponse(code = 500, message = SwaggerMetadata.UNEXPECTED_ERROR)})
	@GET
	@Path("overrideZLiftFeedSpeed/{jobId}/{speed}")
	@Produces(MediaType.APPLICATION_JSON)
	public MachineResponse overrideZLiftFeedSpeed(@PathParam("jobId") String jobId, @PathParam("speed") double speed) {
		UUID uuid = null;
		PrintJob printJob = null;
		try {
			uuid = UUID.fromString(jobId);
			...
			return new MachineResponse("zliftfeedspeed", false, "Invalid jobId: "+ jobId);
		}
		if (printJob == null) {
			return new MachineResponse("zliftfeedspeed", false, "Job:" + jobId + " not found");
		}
			
		try {
			printJob.overrideZLiftFeedSpeed(speed);
		} catch (InappropriateDeviceException e) {
			logger.error("Job:" + jobId + " feed speed:" + speed, e);
			return new MachineResponse("zliftfeedspeed", false, e.getMessage());
		}
		return new MachineResponse("zliftfeedspeed", true, "Set lift feed speed to:" + speed);
	}

	@ApiOperation(value="Overrides the lift retract speed variable for the PrintJob designated by the specified job id.")
	@ApiResponses(value = {
			@ApiResponse(code = 200, message = SwaggerMetadata.SUCCESS),
			@ApiResponse(code = 500, message = SwaggerMetadata.UNEXPECTED_ERROR)})
	@GET
	@Path("overrideZLiftRetractSpeed/{jobId}/{speed}")
	@Produces(MediaType.APPLICATION_JSON)
	public MachineResponse overrideZLiftRetractSpeed(@PathParam("jobId") String jobId, @PathParam("speed") double speed) {
		UUID uuid = null;
		PrintJob printJob = null;
		try {
			uuid = UUID.fromString(jobId);
			...
			return new MachineResponse("zliftretractspeed", false, "Invalid jobId: "+ jobId);
		}
		if (printJob == null) {
			return new MachineResponse("zliftretractspeed", false, "Job:" + jobId + " not found");
		}

		try {
			printJob.overrideZLiftRetractSpeed(speed);
		} catch (InappropriateDeviceException e) {
			logger.error("Job:" + jobId + " retract speed:" + speed, e);
			return new MachineResponse("zliftretractspeed", false, e.getMessage());
		}
		return new MachineResponse("zliftretractspeed", true, "Set lift retract speed to:" + speed);
	}
